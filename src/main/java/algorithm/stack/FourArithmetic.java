 ...
 * 计算后缀表达式(数字入栈,遇到符号则计算栈顶两个元素，结果入栈).
 * 最终栈中的元素就是结果。
 */
public class FourArithmetic {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("直接计算的结果："+ (8+(9-1)*8-7/2));
    ...
    private static String inffixToSuffix(String expression) {
        ...
        while (inffix.length() > 0) {
            ...
            if (isNum(element)) { // 是数字则输出
                ...
                int p1 = priority.get(element);
                int p2 = priority.get(stack.peek());
                while (p1 <= p2 || stack.size() > 0)
                {
